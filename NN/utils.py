...
def compute_masked_loss(pred, target, mask):
    ...
    return loss
def visualize_sample(x_grid, coords, target, pred, mask, epoch, save_dir, 
                     sample_idx=None, filename_prefix="prediction", len_x=20, len_y=5):
    """
    ...
    im0 = axs[0, 0].tripcolor(triang, vel_mag_gt, shading="gouraud", cmap="turbo", vmin=vel_min, vmax=vel_max)
    axs[0, 0].set_ylabel("Velocity Magnitude [m/s]", fontsize=14)
    fig.colorbar(im0, ax=axs[0, 0], fraction=0.046, pad=0.04)
    
    im1 = axs[0, 1].tripcolor(triang, vel_mag_pred, shading="gouraud", cmap="turbo", vmin=vel_min, vmax=vel_max)
    ...
    # Temperature plots
    im3 = axs[1, 0].tripcolor(triang, T_gt, shading="gouraud", cmap="inferno", vmin=T_min, vmax=T_max)
    axs[1, 0].set_ylabel("Temperature [K]", fontsize=14)
    fig.colorbar(im3, ax=axs[1, 0], fraction=0.046, pad=0.04)
    
    im4 = axs[1, 1].tripcolor(triang, T_pred, shading="gouraud", cmap="inferno", vmin=T_min, vmax=T_max)
    ...
    im6 = axs[2, 0].imshow(conductivity_masked, extent=[0, len_x, 0, len_y], 
    ...
    im7 = axs[2, 1].imshow(power_masked, extent=[0, len_x, 0, len_y], 
    ...
    im8 = axs[2, 2].imshow(domain_mask, extent=[0, len_x, 0, len_y], 
